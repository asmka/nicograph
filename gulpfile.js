...
const objectDirs = [releaseDir];

const objectFiles = [zipFile];
...
function buildProdJs() {
  return webpackStream(webpackProdConfig, webpack).pipe(
    gulp.dest(`${releaseDir}/js`)
  );
}

function buildDevJs() {
  return webpackStream(webpackDevConfig, webpack).pipe(
    gulp.dest(`${releaseDir}/js`)
  );
}
...
function copyIcons() {
  return gulp.src(`${iconsDir}/*`).pipe(gulp.dest(`${releaseDir}/icons`));
}
